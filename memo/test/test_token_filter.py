...
def test_filter_by_prefix_strict_match(dummy_vocab_path: str) -> None:
    """前方一致検索が、ターゲットに合致するものだけを許可するかテスト"""
    tf = TokenFilter(vocab_path=dummy_vocab_path)
    # currentが '{"prompt": ' で targetが '{"prompt": "' の場合、次は '"'
    allowed = tf.filter_by_prefix('{"prompt": ', '{"prompt": "')

    assert 1 in allowed  # '"' (id: 1) は許可されるべき
    assert 2 not in allowed  # 'a' (id: 2) は関係ないので弾かれるべき


def test_filter_by_prefix_overshoot_prevention(dummy_vocab_path: str) -> None:
    """目標を飛び越えるような合体トークンを正しくブロックするかテスト"""
    tf = TokenFilter(vocab_path=dummy_vocab_path)
    ...
    assert 12 in allowed  # '}' (id: 12) はピッタリなので許可
    # 合体トークン '}}Ċ' (id: 13) はターゲットを飛び越え（オーバーシュート）
    # になるためブロックされるべき
    assert 13 not in allowed


def test_filter_numeric_tokens(dummy_vocab_path: str) -> None:
    """数値トークンのみを正確に抽出できるかテスト"""
    tf = TokenFilter(vocab_path=dummy_vocab_path)
    allowed_start = tf.filter_numeric_tokens(is_start=True)
    assert 3 in allowed_start  # "1" は許可
    assert 5 in allowed_start  # "." は許可
    # 数字には不要なスペースを許可しない仕様に変更したため not in
    assert 7 not in allowed_start  # " "
    assert 2 not in allowed_start  # "a" (アルファベットは弾かれる)
