...
## **Description**
This project implements a highly optimized, structured decoding and function calling pipeline that strictly regulates LLM outputs based on predefined schemas. Moving beyond basic logit masking, this custom engine introduces "Fast-Forwarding" (Prefix Forcing) to skip boilerplate JSON generation, dramatically boosting inference speed while guaranteeing 100% compliance with a specified JSON format without breaking syntax.
...
To prevent the LLM from ever violating JSON syntax or schema rules while maintaining maximum performance, the pipeline executes the following loop:

Step 1: Dynamic Context Injection
Small-scale language models often suffer from "zombie generation" (e.g., repeating endless 0s or fabricating generic placeholders like "user_name") when trapped inside strict constraints without guidance. To restore the AI's contextual awareness, the System prompt, exact rule sets, and few-shot examples are dynamically pre-pended as a context prefix before every token inference step.

Step 2: Fast-Forwarding (Prefix Forcing)
Instead of forcing the LLM to generate static boilerplate JSON structures (e.g., {"prompt": "...", "name": "), the engine bypasses these steps entirely. The system calculates the mandatory structural prefixes and directly injects them into the input_ids. The LLM is only invoked to predict the dynamic parts: the function names and the parameter values.

Step 3: Logit Masking
OptimizationRather than mutating massive arrays by setting invalid token logits to -inf (a highly CPU-intensive operation over 150,000+ vocabularies), the engine pre-filters a whitelist of allowed_tokens. It then simply selects the token with the maximum logit score exclusively from this small subset (max(valid_ids, key=logits.__getitem__)). This eliminates heavy array manipulations.

Step 4: Cascade State Sync
Immediately after a dynamic value is locked in, the system fast-forwards the internal state to the next required parameter key. This "Cascade" synchronizes the exact structural checkpoint, ensuring the LLM is always positioned precisely at the start of the next value to be generated.  

## Design Decisions
Fast-Forwarding (Prefix Forcing) over Strict Prediction: Relying on the LLM to output predictable JSON syntax brackets and commas introduces unnecessary hallucination risks and processing overhead. Injecting boilerplate strings directly into the model's history forces structural perfection by design.
...
Reliability: By applying optimized logit filtering and prefix forcing, the probability of generating invalid tokens or broken syntax is strictly forced to 0%, ensuring the pipeline outputs 100% valid, parseable JSON consistently.

Accuracy: Even while utilizing a lightweight model like Qwen3-0.6B, the system achieves exceptional accuracy (100% on private evaluation sets) in selecting the correct function and extracting the exact parameters.

Speed: Thanks to lru_cache optimization and the elimination of heavy -inf array masking, the overall vocabulary filtering overhead is drastically reduced. Fast-forwarding further cuts down the required LLM generation steps, allowing the entire test suite to be processed rapidly.
...
Token Stuttering & Semantic Traps: The small 0.6B model exhibited token stuttering (e.g., generating "shrrek" instead of "shrek") and semantic confusion (mistaking template phrases for greeting functions). This was solved by meticulously engineering a 7-step Few-Shot prompt within a strict 79-character line limit (Flake8 compliant) to explicitly guide the LLM's behavioral rules.

Sync Lag Deadlocks: Delays between the AI's output and the state machine's progression resulted in zero allowed tokens (deadlocks). We resolved this by drastically simplifying the state machine to only track FUNCTION_NAME and PARAM_VALUE, instantly fast-forwarding the syntax structure in between.
...
2. **Performance optimizations:** Implemented `@functools.lru_cache`, Fast-Forwarding, and optimized logit selection (eliminating `-inf` array mutation), significantly boosting execution speed.
...
6. **Advanced error recovery mechanisms:** Implemented safe fallbacks, such as mapping irrelevant prompts to an `unknown` function and safely skipping file outputs to prevent crashes. Implemented safe fallbacks, such as mapping irrelevant prompts to an unknown function and safely skipping file outputs to prevent crashes.
...
```
## Design Decisions
* **ボイラープレートの強制ワープ (Fast-Forwarding / Prefix Forcing):**
  JSONの構文（{"prompt": や ,"parameters":{ など）をLLMに推論させることは、幻覚（Hallucination）のリスクと無駄な計算コストを生みます。本システムでは、固定文字列をシステム側で構築してLLMの履歴に強制注入（ワープ）し、LLMには純粋な「関数名」と「値」のみを考えさせる手法を採用しました。
...
* **マスキング計算の最適化:** 不要なトークンすべてに -inf を代入する配列操作を廃止し、許可されたトークン群（ホワイトリスト）の中でのみ最大スコアを検索する方式に変更したことで、計算負荷を極限まで削ぎ落としました。
...
* **トークンの吃音（Stuttering）と意味的トラップ:**
軽量モデル特有の現象として、「shrek」を「shrrek」と出力してしまったり、プロンプト内の特定の単語（"hello"など）に引っ張られて関数を誤認する問題が発生しました。これは、Flake8の1行79文字制限を満たしつつ、7パターンの厳密なFew-Shot例題と絶対ルールをシステムプロンプトに組み込むことで、LLMの幻覚を封じ込めて解決しました。
...
* **同期ラグによるデッドロック:**
  AIの出力に対してステートマシーンの進行が遅れ、許可トークンが0個になる問題は、ステート管理を FUNCTION_NAME と PARAM_VALUE の2つにまで極限にスリム化し、その間をプログラム側で強制補完（ワープ）させることで解決しました。
...
2. **パフォーマンスの最適化 (Performance optimizations):** `@functools.lru_cache` Fast-Forwardingの導入、およびマスキング処理の軽量化（-inf上書きの廃止）により、処理速度を劇的に向上させました。
...
AIにただJSONの穴埋めをさせるだけでは、プロンプトの意図を無視したプレースホルダー（"user_name" など）や無限の 0 を出力する「ゾンビ化」を引き起こします。そのため、毎ターンの推論直前に System プロンプトや利用可能な Functions（関数定義書のJSON）、厳密なルールをテキストの先頭にドッキングし、AIに「タスクの目的」を理解させた上でトークンを予測させます。

### Step 2: 絶対座標ターゲットの構築 (Absolute Target Construction)
{"prompt": "...", "name": " といったJSONの構文（ボイラープレート）をLLMに推論させることはしません。システム側が絶対に必要な構造を計算し、直接LLMの履歴（input_ids）に注入します。LLMは純粋に関数名と値だけを予測します。
### Step 3: トークンのロジット・マスキング (Logit Masking)
ボキャブラリ全体に対して -inf を代入する重い処理は行いません。システム側で「今この瞬間に許されるトークン（allowed_tokens）」のホワイトリストを抽出し、その小さな集合の中だけでLLMの予測スコア（Logits）が最も高いものを選択します。
### Step 4: 連鎖同期ステートマシーン (Cascade State Sync)
LLMが関数名や値を出力し終えた瞬間に、システムは即座に次のパラメータキー（例：,"a":）を強制補完し、次のステートへとワープさせます。これによりAIとプログラムの現在地をミリ単位で同期します。


## **Features / Core Logic**
本プロジェクトのパイプラインは、主に以下の技術とロジックで構成されています．
1. **ステートマシーン (JsonState):**
  JSONの生成プロセスを FUNCTION_NAME と PARAM_VALUE の2つのみに絞り込みました。AIが値を確定した瞬間に、次の状態へと自動で前進させます。
2. **厳密前方一致によるトークンフィルタリング (TokenFilter):**
  AIが目標の文字列を飛び越えてしまうオーバーシュートや、他の関数の誤検知を防ぐため、常にその状態における座標のターゲット文字列を構築．現在地がその接頭辞（prefix）と完全に一致するルートのトークンのみを許可（ホワイトリスト化）します．
3. **動的コンテキスト注入:**
...
* **Byte Pair Encoding**
  1. LLMの辞書（vocab.json）の中には、`a`、`ap`、`app`、`apple` のように、短い文字列から長い文字列まで様々なトークンが登録されています。
     もしこれを「適当な順番」や「短い順」でマッチングしてしまうと,"apple" という単語を見たとき、a + p + p + l + e のように、1文字ずつバラバラの5トークンに分解されてしまいます。
     LLMの脳内: LLMは学習時、よく使う単語は「できるだけ長い塊（最長一致）」で区切って覚えています（これをBPE: Byte Pair Encodingと呼びます）。そのため、apple という1トークンとして渡してあげないと、「私の知らない宇宙語が来た！」とLLMが混乱してしまい、まともな推論ができなくなります。
  2. 「強制ワープ」の同期ズレを防ぐ
    今回の最強コードでは、{"prompt":"... のような固定文字列をPython側で作って、LLMの履歴（input_ids）に強制的に追加している。
    このとき、Python側で追加した文字列をトークンIDに変換するわけですが、ここで最長一致を使わないと「LLMが自分で出力したときのトークンの区切り方」と「Python側が無理やり足したトークンの区切り方」にズレが生じてしまう
    このズレが起きると、LLMは「あれ？なんか自分が喋ってた言葉の区切りがおかしいぞ？」と違和感を感じ、そこから幻覚（Hallucination）を見始めたり、出力をストップしてしまったりします。
    最長一致アルゴリズム（Greedy matching）は、LLM本来のトークナイザーの挙動を最も忠実に再現できるため、この「違和感のない強制ワープ」を実現するために絶対に欠かせないロジックなのです。
...
*   **Check that all test prompts are processed in reasonable time (<5 minutes)**
    ...
    *   *(テスト)* `make run` を実行し、完了までの時間が１プロンプト仁木5分を下回っていることを確認します。これは `lru_cache` などの最適化によるものです。
